          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins" @click="skuNum===0?0:skuNum--">-</a>
              </div>
              <div class="add">
                <!-- 点击加入购物车需要先发送请求给后台，后台需要把购物车存储到数据库，请求成功后会返回信息 -->
...
export default {
    ...
    methods:{
      ...
      changeChecked(spuSaleAttrValueList,spuSaleAttrValue){
        // spuSaleAttrValueList当前所有的属性值列表
        // spuSaleAttrValue当前点击的属性值
      ...
      async addShopCart(){
        try{
          // 1.发送请求添加购物车,dispatch就是在调用指定的函数，函数的返回值为Promise
          await this.$store.dispatch('shopcart/getAddToShopCart',{skuId:this.skuId,skuNum:this.skuNum});
          alert('添加购物车成功！');
          // 2.根据返回信息决定是否跳转到添加购物车成功页面
          // 请求成功跳转到添加购物车成功页面，并携带skuNum和商品详情信息
