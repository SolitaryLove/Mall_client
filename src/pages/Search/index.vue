...
export default {
    ...
    data(){
      return {
        searchParams:{
          ...
          order: "1:desc", // 排序规则
          pageNo: 1, // 搜索第几页商品
          pageSize: 10, // 每页显示多少商品
        }
      }
    },
    beforeMount(){
      // 在点击三级分类列表或者点击搜索按钮跳转过来发送请求之前
      // 将对应的三级分类列表id和name或者搜索框keyword赋值到searchParams中
    ...
    methods:{
      ...
      handlerSearchParams(){
          ...
          Object.keys(searchParams).forEach(key=>{
              if(speechSynthesis[key]===''){delete searchParams[key];}
          });
      ...
      changeSort(sortFlag){
        ...
        if(sortFlag===originSortFlag){
            // 用户点击的排序标志与原来一样，只需要改变排序类型
            newOrder=`${originSortFlag}:${originSortType==='asc'?'desc':'asc'}`;
        }else{
            // 用户点击的排序标志与原来不一样，只需要改变排序标志，排序类型默认
    ...
    computed:{
      ...
      // 排序标志
      sortFlag(){return this.searchParams.order.split(':')[0]},
      // 排序类型
      sortType(){return this.searchParams.order.split(':')[1]},
    },
    watch:{
      // 路径改变时重新发送请求
      $route(){
